...
## DOM
Your test run on node, node does not have the DOM api like the browser does.  In test you have to fake the dom.  Jest uses JSDOM, vitest uses `JSDOM` or `HAPPY DOM` (Small and Lightweight) .  Vitest allows the user to choose between JSDOM or HAPPY DOM.
